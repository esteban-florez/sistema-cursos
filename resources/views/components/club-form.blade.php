...
<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="row d-flex align-items-center">
    ...
    <div class="col-sm-6 col-md-8">
      ...
      <x-select name="day" id="day" :options="['' => 'Seleccionar...', 'mo' => 'Lunes', 'tu' => 'Martes', 'we' => 'Miércoles', 'Th' => 'Jueves', 'fr' => 'Viernes', 'sa' => 'Sábado', 'su' => 'Domingo']" :selected="old('day') ?? $club->day ?? ''" required>
        Días:
      </x-select>
    </div>
    <div class="col-sm-6">
      <x-field type="time" name="start_hour" id="startHour" value="{{ old('start_hour') ?? $club->start_hour ?? '' }}" required>
        Hora de Inicio:
      </x-field>
    </div>
    <div class="col-sm-6">
      <x-field type="time" name="end_hour" id="endHour" value="{{ old('end_hour') ?? $club->end_hour ?? '' }}" required>
        Hora de Cierre:
      </x-field>
    </div>
    <div class="col-12">
